...
tmp12::Cube::Cube(float X, float Y, float Z, float Size, int type, float Pitch, float Yaw)
{
	x = X;
	y = Y;
	z = Z;
...
void tmp12::Cube::initCube()
{
	if (textureLiberyID == NULL)
	{
		...
		unsigned char* texture = stbi_load("resources/terrain.png", &width, &height, &bpp, 4);
